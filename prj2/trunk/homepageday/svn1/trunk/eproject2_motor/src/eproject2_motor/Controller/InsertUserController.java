   ...
   @FXML
    ...
    void btnSubmit(ActionEvent event) throws IOException {
        if (this.editAdmin == null) {
            User insertAdmin = extract();
            insertAdmin = userDao.insert(insertAdmin);
            String msg = "Insert complete!!!";
//            lbMess.setText(msg);
        } else {
            User updateAdmin = extract();
            updateAdmin.setId(editAdmin.getId());
            boolean result = userDao.update(updateAdmin);
//            if (result) {
...
//            }
    ...
    public void initialize(User editAdmin) {
